...
const isRecurrence = (
    ...
    // Get the year, month, day strings without the time
    const [yearAsString, monthAsString, dayAsString] = iso8601Date.split("T")[0].split("-");
    
    // Parse the year, month, day as base-10 digits
    const year = parseInt(yearAsString, 10);
    const month = parseInt(monthAsString, 10) - 1; // Subtract 1 because January starts at value '0'
    const day = parseInt(dayAsString, 10);

    // Check if the provided date matches the recurrence rule
    const date = new Date(year, month, day); 
    return recurrenceRule.between(date, date, true).length > 0 
        ? true 
        : false;
};
